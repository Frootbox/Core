...
class CategoryConnection extends \Frootbox\Persistence\CategoryConnection implements \Frootbox\Persistence\Interfaces\MultipleAliases
{
    ...
    {
        // Fetch category
        $categoriesRepository = $this->db->getRepository(\Frootbox\Ext\Core\News\Persistence\Repositories\Categories::class);
        $category = $categoriesRepository->fetchById($this->getCategoryId());

        // Fetch article
        $articlesRepository = $this->db->getRepository(\Frootbox\Ext\Core\News\Persistence\Repositories\Articles::class);
        $article = $articlesRepository->fetchById($this->getItemId());
        ...
        $virtualDirectory[] = $article->getDateStart()->format('%Y');
        $virtualDirectory[] = $article->getDateStart()->format('%m');
        $virtualDirectory[] = $article->getTitle();
